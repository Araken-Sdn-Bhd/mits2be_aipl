...
class CpsProgressNoteController extends Controller
{
            ...
            if ($request->id) {
                ...
                if ($request->appId == null || $request->appId == '') {
                    ...
                    if ($checkTodayAppointment) {
                        ...
                        $PatientDetails = PatientRegistration::where('id', $request->patient_mrn_id)->orWhere('patient_mrn', $request->patient_mrn_id)->first();
    ...
    public function storeMobile(Request $request)
    {
        ...
        if ( $request->add_diagnosis_type1 != null && $request->add_diagnosis_type1 != ''){
            ...
            if ($request->diagnosis_type2 != null && $request->diagnosis_type2 != ''){
                $additional_diagnosis .= ','.$request->diagnosis_type2;
            }
            if ($request->diagnosis_type3 != null && $request->diagnosis_type3 != ''){
                $additional_diagnosis .= ','.$request->diagnosis_type3;
            }
            if ($request->diagnosis_type4 != null && $request->diagnosis_type4 != ''){
                $additional_diagnosis .= ','.$request->diagnosis_type4;
            }
            if ($request->diagnosis_type5 != null && $request->diagnosis_type5 != ''){
                $additional_diagnosis .= ','.$request->diagnosis_type5;
            }
        }
        ...
        if ($request->appId == null || $request->appId == '') {
            ...
            if ($checkTodayAppointment) {
                ...
                $PatientDetails = PatientRegistration::where('id', $request->patient_mrn_id)->where('patient_mrn', 'like', '%'.$request->patient_mrn_id.'%')->first();
                if ($PatientDetails->nric_no != null || $PatientDetails->nric_no != '') {
                    $nric_or_passportno = $PatientDetails->nric_no;
                } else if ($PatientDetails->passport_no != null && $PatientDetails->nric_no == null && $PatientDetails->nric_no != '') {
                    $nric_or_passportno = $PatientDetails->passport_no;
